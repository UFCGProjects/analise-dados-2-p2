...
term_word_count <- read.csv("~/git/analise-dados-2-p2/csv/term_word_count.csv")
```
...
```{r, echo=FALSE}
term_word_count <- read.csv("~/git/analise-dados-2-p2/csv/term_word_count.csv")
ggparcoord(data = term_word_count[1:6], columns = 2:6, order = "allClass", groupColumn = 1, mapping = aes(size = 2))
```
